...
func (s *RoomUserRepository) FindActiveRoomForUser(userUUID string) (*entity.RoomUser, error) {
	...
	err := s.db.QueryRow(`
		...
		LIMIT 1
	`, userUUID).Scan(s.getEntityFieldPointers(&roomUser, &rawShipsSchema)...)
	...
	roomUser.ShipsSchema, err = s.parseRawShipsSchema(rawShipsSchema)
...
func (s *RoomUserRepository) GetByRoomUuid(roomUUID string) ([]*entity.RoomUser, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(s.getEntityFieldPointers(&roomUser, &rawShipsSchema)...)
...
func (s *RoomUserRepository) Update(roomUser *entity.RoomUser) error {
	rawShipsSchema, err := s.transformShipsSchemaIntoRawValue(roomUser)
	...
	_, err = s.db.Exec(`
		...
		SET
			...
			abandoned_at = ?
		WHERE room_uuid = ? AND user_uuid = ?
	`, rawShipsSchema, roomUser.JoinedAt, roomUser.WonAt, roomUser.LostAt, roomUser.AbandonedAt, roomUser.RoomUUID, roomUser.UserUUID)
...
func (s *RoomUserRepository) FindRandomActivePlayer(roomUUID string) (*entity.RoomUser, error) {
	...
	err := s.db.QueryRow(`
		...
		ORDER BY RANDOM()
		LIMIT 1
	`, roomUUID).Scan(s.getEntityFieldPointers(&roomUser, &rawShipsSchema)...)
...
func (s *RoomUserRepository) getEntityFieldPointers(roomUser *entity.RoomUser, rawShipsSchema any) []any {
	return []any{
		&roomUser.RoomUUID,
		&roomUser.UserUUID,
		rawShipsSchema,
...
func (s *RoomUserRepository) transformShipsSchemaIntoRawValue(roomUser *entity.RoomUser) (string, error) {
	if roomUser.ShipsSchema == nil {
		return "", nil
	}

	j, err := json.Marshal(roomUser.ShipsSchema)
...
func (s *RoomUserRepository) parseRawShipsSchema(rawShipsSchema string) (*entity.Schema, error) {
	if rawShipsSchema == "" {
		return nil, nil
	}
	...
	err := json.Unmarshal([]byte(rawShipsSchema), &schema)
