...
func (s *RoomRepository) Store(room *entity.Room) error {
	...
	_, err := s.db.Exec(`
		INSERT INTO rooms (uuid, name, map_height, map_width, max_players, current_players, created_by, created_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?)
	`, u, room.Name, room.MapHeight, room.MapWidth, room.MaxPlayers, room.CurrentPlayers, room.CreatedBy, createdAt)
